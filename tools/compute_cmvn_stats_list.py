...
class CollateFunc(object):
    ...
    def __init__(self, feat_dim, feat_type, resample_rate, input_format='scp'):
...
class AudioDataset(Dataset):
    def __init__(self, data_file, input_format='scp'):
        self.items = []
        self.input_format = input_format
            ...
            for line in f:
                ...
                if input_format == 'scp':
                    ...
                    if len(arr) >= 2:
                        ...
                        print(f"Warning: Invalid scp format line: {line}", file=sys.stderr)
                else:  # list格式
                    # 处理JSON格式
                    try:
                        data = json.loads(line)
                        # 检查必需的字段
                        if 'key' in data and 'wav' in data:
                            self.items.append(data)
                        else:
                            print(f"Warning: Missing required fields in line: {line}", file=sys.stderr)
                    except json.JSONDecodeError as e:
                        print(f"Error parsing JSON line: {line}, error: {e}", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_scp', 
                        default=None, 
                        help='wav scp file (format: key wav_path)')
    parser.add_argument('--in_list',
                        default=None,
                        help='wav list file in JSON format')
    ...
    # 检查输入文件参数
    if args.in_scp and args.in_list:
        print("Error: Cannot specify both --in_scp and --in_list", file=sys.stderr)
        sys.exit(1)
    if not args.in_scp and not args.in_list:
        print("Error: Must specify either --in_scp or --in_list", file=sys.stderr)
        sys.exit(1)
    
    # 确定输入格式
    if args.in_scp:
        input_file = args.in_scp
        input_format = 'scp'
    else:
        input_file = args.in_list
        input_format = 'list'
    ...
    collate_func = CollateFunc(feat_dim, feat_type, resample_rate, input_format)
    dataset = AudioDataset(input_file, input_format)
    batch_size = 20
